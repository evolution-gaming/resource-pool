...
class ResourcePoolTest extends AsyncFunSuite with Matchers {
  ...
  test("allocate multiple resources in parallel as long as it does not exceed `maxSize`") {
    val resource = for {
      ...
      pool <- {
        ...
        result.toResource
          .toResourcePool(maxSize = 2, expireAfter = 1.day)
      }
    } yield for {
  ...
  test("expire after use") {
    val result = for {
      ...
      pool <- Resource
        ...
        .toResourcePool(maxSize = 5, expireAfter = 10.millis)
  ...
  test("not exceed `maxSize`") {
    ...
    val resource = for {
      ...
      pool <- ref
        .update(_ + 1)
        .toResource
        .toResourcePool(maxSize = maxSize, expireAfter = 1.day)
    } yield for {
      _ <- pool.resource
  ...
  test("cancel `get` while waiting in queue") {
    val result = for {
      ...
      pool <- ()
        .pure[Resource[IO, *]]
        .toResourcePool(maxSize = 1, expireAfter = 1.day)
    } yield for {
      fiber0 <- pool.resource.use { _ =>
        deferred0
          .complete(())
          .productR(IO.never[Unit])
      }.start
      _      <- deferred0.get
      fiber1 <- pool.get.start
      result <- fiber1.joinWithNever
        .timeout(10.millis)
        .attempt
      _ <- IO(result should matchPattern { case Left(_: TimeoutException) => })
  ...
  test("cancel `resource` while waiting in queue") {
    val result = for {
      ...
      _      <- deferred0.get
      fiber1 <- pool.resource.use(_ => IO.never[Unit]).start
      result <- fiber1.joinWithNever
        .timeout(10.millis)
        .attempt
      _ <- IO(result should matchPattern { case Left(_: TimeoutException) => })
  ...
  test("cancel `resource.use") {
    val result = for {
      ...
      result <- deferred0
        ...
        .allocated
      (pool, release) = result
      fiber0         <- pool.resource.use(_ => IO.never).start
      result <- fiber0.joinWithNever
        .timeout(10.millis)
        .attempt
      _      <- IO(result should matchPattern { case Left(_: TimeoutException) => })
  ...
  test("release before resource allocation is completed") {
    val result = for {
      deferred <- Deferred[IO, Unit]
      result <- deferred.get.toResource
        .toResourcePool(maxSize = 1, expireAfter = 1.day)
        .allocated
      (pool, release) = result
      fiber0         <- pool.resource.use(_.pure[IO]).start
  ...
  test("discard tasks on release") {
    val result = for {
      result <- ()
        .pure[Resource[IO, *]]
        .toResourcePool(maxSize = 1, expireAfter = 1.day, discardTasksOnRelease = true)
        .allocated
      (pool, release) = result
      fiber           = pool.resource.use(_ => IO.never[Unit]).start
      fiber0         <- fiber
      result <- fiber0.joinWithNever
        .timeout(10.millis)
        .attempt
